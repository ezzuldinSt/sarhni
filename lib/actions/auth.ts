...
export async function loginUser(prevState: any, formData: FormData) {
  ...
  try {
    ...
    // Prevent username enumeration: check ban status WITH password validation
    // This ensures we don't reveal if a username exists but is banned
    if (!user || !user.password || user.isBanned || !await bcrypt.compare(password, user.password)) {
