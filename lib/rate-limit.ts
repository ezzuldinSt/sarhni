...
/**
 * Centralized IP extraction utility that trusts only Vercel's forwarded header
 * This prevents IP spoofing attacks where users set x-forwarded-for or x-real-ip headers
 *
...
export async function getClientIP(): Promise<string> {
  ...
  // Trust ONLY Vercel's header (set by Vercel's infrastructure, not user-controlled)
  const ip = headerList.get("x-vercel-forwarded-for") || "unknown";
 ...
 * Survives cold starts and prevents IP spoofing bypass
 *
 * @param userId - The user ID to rate limit
 * @param maxRequests - Maximum requests allowed (default: 10 per hour)
...
export function checkRateLimitByUser(
  ...
  const now = Date.now();
  const identifier = `user:${userId}`; // Prefix to avoid conflicts with IP-based limiting
  const record = rateLimitMap.get(identifier);

  // 1. New user? Track them.
  if (!record) {
